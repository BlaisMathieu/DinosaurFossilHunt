...
public class PterodactylPopulator extends BlockPopulator {

    private int SpawnChance = 30000000;

    public PterodactylPopulator() {}

    public void generate(Block block) {
        Location loc = block.getLocation();
        try {
            SchematicLoading schem = new SchematicLoading("C:\\Users\\Mathieu\\Desktop\\serv\\plugins\\WorldEdit\\schematics\\GOODptero.schem");
            ...
            Bukkit.broadcastMessage("Nouveau fossile de pterodactyl : " + loc.getX() + "   " + loc.getZ());
            ...
            for (int y = 1; y < 70; y++) {
                for (int z = 0; z < 16; z++) {
                    ...
                    if (r == 3 && block.getType().compareTo(Material.STONE) == 0) {
                        if (block.getBiome().equals(Biome.MOUNTAINS) || block.getBiome().equals(Biome.MOUNTAIN_EDGE)
                            ...
                            generate(block);
                        else if (block.getBiome().equals(Biome.SNOWY_MOUNTAINS) || block.getBiome().equals(Biome.SAVANNA) || block.getBiome().equals(Biome.SAVANNA_PLATEAU) || block.getBiome().equals(Biome.SHATTERED_SAVANNA)
                                || block.getBiome().equals(Biome.SHATTERED_SAVANNA_PLATEAU)) {
                            int t = (int) (Math.random() * 2);
                            if (t == 1 && block.getType().compareTo(Material.STONE) == 0)
                                generate(block);
                        }
                        else if (block.getBiome().equals(Biome.JUNGLE) || block.getBiome().equals(Biome.MODIFIED_JUNGLE)
                                || block.getBiome().equals(Biome.JUNGLE_HILLS)) {
