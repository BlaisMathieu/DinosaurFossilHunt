...
public class SaurophaganaxPopulator extends BlockPopulator {

    private int SpawnChance = 30000000;

    public SaurophaganaxPopulator() {}

    public void generate(Block block) {
        Location loc = block.getLocation();
        try {
            SchematicLoading schem = new SchematicLoading("C:\\Users\\Mathieu\\Desktop\\serv\\plugins\\WorldEdit\\schematics\\GOODsauro.schem");
            ...
            Bukkit.broadcastMessage("Nouveau fossile de saurophaganax : " + loc.getX() + "   " + loc.getZ());
            ...
            for (int y = 1; y < 70; y++) {
                for (int z = 0; z < 16; z++) {
                    ...
                    if (r == 3 && block.getType().compareTo(Material.STONE) == 0) {
                        if (block.getBiome().equals(Biome.BADLANDS) || block.getBiome().equals(Biome.BADLANDS_PLATEAU) || block.getBiome().equals(Biome.ERODED_BADLANDS)
                            ...
                            generate(block);
                        else if (block.getBiome().equals(Biome.DESERT) || block.getBiome().equals(Biome.DESERT_HILLS)
